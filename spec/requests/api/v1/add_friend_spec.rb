...
RSpec.describe "Add Friend" do 
  ...
  describe "sad path" do 
    it "can handle error for adding self as friend" do 
      ...
      expect(reply[:message]).to eq("Friend can't be the same as user, Friend can't be the same as user")
    ...
    it "can handle error for adding friend that is already a friend" do 
      ...
      expect(reply[:message]).to eq("User has already been taken")
      expect(reply[:status]).to eq("400")
    end
    ...
    it "can handle error for adding non existent user as friend" do 
      ...
      expect(reply[:errors]).to eq([{:message=>"Couldn't find User with 'id'=12345678", :status=>"404"}])
    end
  end
end
